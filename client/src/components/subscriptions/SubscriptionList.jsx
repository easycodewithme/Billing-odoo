...
const STATUS_TABS = [
  ...
  { value: SUBSCRIPTION_STATUS.QUOTATION, label: 'Quotation' },
  { value: SUBSCRIPTION_STATUS.CONFIRMED, label: 'Confirmed' },
...
export default function SubscriptionList({ refreshKey }) {
  ...
  const actions = (row) => {
    const items = [
      { label: 'View', icon: Eye, onClick: () => navigate(`/subscriptions/${row.id}`) },
    ];
    if (isStaff && (row.status === 'draft' || row.status === 'quotation')) {
      items.push({ label: 'Edit', icon: Pencil, onClick: () => navigate(`/subscriptions/${row.id}`) });
    }
    return items;
  };
